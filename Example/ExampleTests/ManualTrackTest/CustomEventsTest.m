...
- (void)test1TrackNormal {
    ...
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:@"GrowingIO2018"];
...
- (void)test2SpecialChar {
     ...
     **/
    [MockEventQueue.sharedQueue cleanQueue];
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:@"GIO%#*/"];
    [tester waitForTimeInterval:2];
    NSArray *cstmEventArray = [MockEventQueue.sharedQueue eventsFor:@"CUSTOM"];
    // NSLog(@"Cstm事件：%@",cstmEventArray);
    if (cstmEventArray.count >= 1) {
...
- (void)test3ChineseChar {
     ...
     **/
    [MockEventQueue.sharedQueue cleanQueue];
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:@"企业增长"];
    [tester waitForTimeInterval:2];
    NSArray *cstmEventArray = [MockEventQueue.sharedQueue eventsFor:@"CUSTOM"];
    // NSLog(@"Cstm事件：%@",cstmEventArray);
    if (cstmEventArray.count >= 1) {
...
- (void)test5EventIDNil {
    ...
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:NULL];
    //检测日志输出
    Boolean chres = [LogOperHelper CheckLogOutput:[LogOperHelper getFlagErrNsLog]];
    //恢复日志重定向
    [LogOperHelper redirectLogBack];
...
- (void)test6EventIDEmpty {
    ...
    [LogOperHelper writeLogToFile];
    //    [[viewTester usingLabel:@"Track请求"] tap];
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:@""];
    //检测日志输出
    Boolean chres = [LogOperHelper CheckLogOutput:[LogOperHelper getFlagErrNsLog]];
    //恢复日志重定向
    [LogOperHelper redirectLogBack];
...
- (void)test9WithNumKeyNil {
    ...
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:NULL withAttributes:@{@"num" : @"企业增长"}];
    //检测日志输出
    Boolean chres = [LogOperHelper CheckLogOutput:[LogOperHelper getFlagErrNsLog]];
    //恢复日志重定向
    [LogOperHelper redirectLogBack];
...
- (void)test10WithNumKeyEmpty {
    ...
    [LogOperHelper writeLogToFile];
    //    [[viewTester usingLabel:@"TrackNum"] tap];
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:@"" withAttributes:@{@"num" : @"企业增长"}];
    //检测日志输出
    Boolean chres = [LogOperHelper CheckLogOutput:[LogOperHelper getFlagErrNsLog]];
    //恢复日志重定向
    [LogOperHelper redirectLogBack];
...
- (void)test18WithVariable {
     ...
     **/
    [MockEventQueue.sharedQueue cleanQueue];
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:@"GIO" withAttributes:@{@"name" : @"GIO", @"title" : @"QA"}];
...
- (void)test19WithVariableUpdate {
     ...
     **/
    [MockEventQueue.sharedQueue cleanQueue];
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:@"GIO" withAttributes:@{@"name" : @"GrowingIO", @"title" : @"RD"}];
    [tester waitForTimeInterval:2];
    NSArray *cstmEventArray = [MockEventQueue.sharedQueue eventsFor:@"CUSTOM"];
    // NSLog(@"Cstm事件：%@",cstmEventArray);
    if (cstmEventArray.count >= 1) {
...
- (void)test20WithVariableNil {
    ...
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:@"GIO" withAttributes:NULL];
...
- (void)test22TrackChinese {
     ...
     **/
    [MockEventQueue.sharedQueue cleanQueue];
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:@"北京"];
    [tester waitForTimeInterval:2];
    NSArray *cstmEventArray = [MockEventQueue.sharedQueue eventsFor:@"CUSTOM"];
    // NSLog(@"Cstm事件：%@",cstmEventArray);
    if (cstmEventArray.count >= 1) {
...
- (void)test23WithVariableEventidNil {
    ...
    [LogOperHelper writeLogToFile];
    //   [[viewTester usingLabel:@"TrackWV"] tap];
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:NULL withAttributes:@{@"name" : @"GrowingIO"}];
    //检测日志输出
    Boolean chres = [LogOperHelper CheckLogOutput:[LogOperHelper getFlagErrNsLog]];
    //恢复日志重定向
    [LogOperHelper redirectLogBack];
...
- (void)test24WithVariableEventidEmpty {
    ...
    [LogOperHelper writeLogToFile];
    //    [[viewTester usingLabel:@"TrackWV"] tap];
    [[GrowingCdpTracker sharedInstance] trackCustomEvent:@"" withAttributes:@{@"name" : @"GrowingIO"}];

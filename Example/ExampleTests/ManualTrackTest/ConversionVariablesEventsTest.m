...
- (void)test1SetEvarNormal {
    ...
    [[GrowingCdpTracker sharedInstance] setConversionVariables:@{@"var1" : @"good", @"var2" : @"excell"}];
...
- (void)test2SetEvarNil {
    ...
    [[GrowingCdpTracker sharedInstance] setConversionVariables:nil];
    //检测日志输出
...
- (void)test3SetEvarEmpty {
     ...
     **/
    [MockEventQueue.sharedQueue cleanQueue];
    [[GrowingCdpTracker sharedInstance] setConversionVariables:@{}];
    [tester waitForTimeInterval:3];
    NSArray *evarEventArray = [MockEventQueue.sharedQueue eventsFor:@"CONVERSION_VARIABLES"];
    // NSLog(@"CONVERSION_VARIABLES事件：%@",evarEventArray);
...
- (void)test4SetEvarKeyStr {
     ...
     **/
    [MockEventQueue.sharedQueue cleanQueue];
    [[GrowingCdpTracker sharedInstance] setConversionVariables:@{@"ekey1" : @"Good"}];
    [tester waitForTimeInterval:3];
    NSArray *evarEventArray = [MockEventQueue.sharedQueue eventsFor:@"CONVERSION_VARIABLES"];
    // NSLog(@"CONVERSION_VARIABLES事件：%@",evarEventArray);
    if (evarEventArray.count >= 1) {
...
- (void)test2SetEvarKeyStrUpdate {
     ...
     **/
    [MockEventQueue.sharedQueue cleanQueue];
    [[GrowingCdpTracker sharedInstance] setConversionVariables:@{@"ekey1" : @"Better"}];
    [tester waitForTimeInterval:3];
    NSArray *evarEventArray = [MockEventQueue.sharedQueue eventsFor:@"CONVERSION_VARIABLES"];
    // NSLog(@"CONVERSION_VARIABLES事件：%@",evarEventArray);
    if (evarEventArray.count >= 1) {
...
- (void)test6SetEvarKeyEmpty {
    ...
    [[GrowingCdpTracker sharedInstance] setConversionVariables:@{@"" : @"Better"}];
    //检测日志输出
    Boolean chres = [LogOperHelper CheckLogOutput:[LogOperHelper getFlagErrNsLog]];
    //恢复日志重定向
    [LogOperHelper redirectLogBack];
...
- (void)test7SetEvarKeyNil {
    ...
    [LogOperHelper writeLogToFile];
    //    [[viewTester usingLabel:@"SendESVar"] tap];
    [[GrowingCdpTracker sharedInstance] setConversionVariables:@{[NSNull null] : @"Better"}];
    //检测日志输出
...
- (void)test9SetEvarValStrEmpty {
    ...
    [LogOperHelper writeLogToFile];
    //    [[viewTester usingLabel:@"SendESVar"] tap];
    [[GrowingCdpTracker sharedInstance] setConversionVariables:@{@"ekey1" : @""}];
    //检测日志输出
    Boolean chres = [LogOperHelper CheckLogOutput:[LogOperHelper getValueErrNsLog]];
    //恢复日志重定向
    [LogOperHelper redirectLogBack];
...
- (void)test10SetEvarValNum {
     ...
     **/
    [MockEventQueue.sharedQueue cleanQueue];
    [[GrowingCdpTracker sharedInstance] setConversionVariables:@{@"evkey1" : @"132"}];
    [tester waitForTimeInterval:3];
    NSArray *evarEventArray = [MockEventQueue.sharedQueue eventsFor:@"CONVERSION_VARIABLES"];
    // NSLog(@"CONVERSION_VARIABLES事件：%@",evarEventArray);
    if (evarEventArray.count >= 1) {
...
- (void)test11SetEvarValNumUpdate {
     ...
     **/
    [MockEventQueue.sharedQueue cleanQueue];
    [[GrowingCdpTracker sharedInstance] setConversionVariables:@{@"evkey1" : @"43.22"}];
    [tester waitForTimeInterval:3];
    NSArray *evarEventArray = [MockEventQueue.sharedQueue eventsFor:@"CONVERSION_VARIABLES"];
    // NSLog(@"CONVERSION_VARIABLES事件：%@",evarEventArray);
    if (evarEventArray.count >= 1) {
...
- (void)test12SetEvarValNumKeyEmpty {
    ...
    [[GrowingCdpTracker sharedInstance] setConversionVariables:@{@"" : @"43.22"}];
    //检测日志输出
    Boolean chres = [LogOperHelper CheckLogOutput:[LogOperHelper getFlagErrNsLog]];
    //恢复日志重定向
    [LogOperHelper redirectLogBack];
...
- (void)test14SetEvarValNumValEmpty {
    ...
    [LogOperHelper writeLogToFile];
    //   [[viewTester usingLabel:@"SendENVar"] tap];
    [[GrowingCdpTracker sharedInstance] setConversionVariables:@{@"enkey2" : @""}];
    //检测日志输出
    Boolean chres = [LogOperHelper CheckLogOutput:[LogOperHelper getValueErrNsLog]];
    //恢复日志重定向
    [LogOperHelper redirectLogBack];
...
- (void)test16SetEvarKeyStrChinese {
     ...
     **/
    [MockEventQueue.sharedQueue cleanQueue];
    [[GrowingCdpTracker sharedInstance] setConversionVariables:@{@"关键字" : @"北京"}];
    [tester waitForTimeInterval:3];
    NSArray *evarEventArray = [MockEventQueue.sharedQueue eventsFor:@"CONVERSION_VARIABLES"];
    // NSLog(@"CONVERSION_VARIABLES事件：%@",evarEventArray);
    if (evarEventArray.count >= 1) {
